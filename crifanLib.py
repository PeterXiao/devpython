...
def decodeHtmlEntity(origHtml, decodedEncoding=""):
    ...
    decodedCodepointHex = re.sub('&#x(?P<codePointHex>[a-fA-F\d]{2,5});',
                                 lambda matched: unichr(int(matched.group("codePointHex"), 16)), decodedCodepointInt);
...
def convertToTupleVal(equationStr):
    ...
    try:
        ...
        (key, value) = ('', None);
        print
        "Fail of convert the equal string %s to value" % (equationStr);
...
def printCurrentCookies():
    ...
    if (gVal['cj']):
        for index, cookie in enumerate(gVal['cj']):
            print
            "[%d] name=%s,value=%s,domain=%s,path=%s,secure=%s,expires=%s,version=%d" % \
            (index, cookie.name, cookie.value, cookie.domain, cookie.path, cookie.secure, cookie.expires,
             cookie.version);
...
def downloadFile(fileUrl, fileToSave, needReport=False):
    ...
    def reportHook(copiedBlocks, blockSize, totalFileSize):
        ...
        if copiedBlocks == 0: # 1st call : once on establishment of the network connection
            print
            'Begin to download %s, total size=%d' % (downloadingFile, totalFileSize);
        else: # rest call : once after each block read thereafter
            print
            'Downloaded bytes: %d\r' % ( blockSize * copiedBlocks),;
        return;
    ...
    try:
        if fileUrl:
            ...
            print
            "Input download file url is NULL";
...
def getUrlRespHtml_multiTry(url, postDict={}, headerDict={}, timeout=0, useGzip=True, postDataDelimiter="&",
                            maxTryNum=5):
    """
...
def translateString(strToTranslate, fromLanguage="zh-CN", toLanguage="en"):
    ...
    try:
        ...
        postDict = {'hl': 'zh-CN', 'ie': 'UTF-8', 'text': strToTranslate,
                    'langpair': "%s|%s" % (fromLanguage, toLanguage)};
